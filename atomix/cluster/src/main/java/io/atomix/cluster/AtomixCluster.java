...
public class AtomixCluster implements BootstrapService, Managed<Void> {
  ...
  public AtomixCluster(final ClusterConfig config, final Version version, final OpenTelemetry openTelemetry) {
    this(config, version, buildMessagingService(config, openTelemetry), buildUnicastService(config), openTelemetry);
  }
   ...
   * @return a new Atomix builder
   */
  public static AtomixClusterBuilder builder(final ClusterConfig config, final OpenTelemetry openTelemetry) {
  ...
  protected static ManagedMessagingService buildMessagingService(final ClusterConfig config, final OpenTelemetry openTelemetry) {
    return new NettyMessagingService(
        config.getClusterId(), config.getNodeConfig().getAddress(), config.getMessagingConfig(), openTelemetry);
  }

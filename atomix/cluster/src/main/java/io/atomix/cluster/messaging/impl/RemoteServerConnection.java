...
final class RemoteServerConnection extends AbstractServerConnection {
  ...
  @Override
  public void reply(final ProtocolRequest message, final Status status, final Optional<byte[]> payload) {
    try (final Scope current =
        openTelemetry
            ...
            .makeCurrent()) {
      final ProtocolReply response = new ProtocolReply(message.id(), payload.orElse(EMPTY_PAYLOAD), status, message.metadata());
      final Tracer tracer =
